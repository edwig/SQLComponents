SELECT cast('' as varchar(255))                   as table_cat                -- 1  - VARCHAR
      ...
      ,cast(rfr.rdb$relation_name as varchar(31)) as table_name	              -- 3  - VARCHAR NOT NULL
      ,cast(rfr.rdb$field_name    as varchar(31)) as column_name		          -- 4  - VARCHAR NOT NULL
      ...
      ,(coalesce(rfr.rdb$null_flag,0,0)-1)*-1     as nullable        				  -- 11 - SMALLINT NOT NULL
      ,cast (rfr.rdb$description as varchar(512)) as remarks                  -- 12 - VARCHAR
      ,cast (rfr.rdb$default_source as varchar(512)) as column_def         		-- 13 - VARCHAR
      ,CASE fld.rdb$field_type
            WHEN 7  THEN CASE fld.rdb$field_sub_type
                              WHEN 1 THEN 2
                              WHEN 2 THEN 2
      ...
      ,rfr.rdb$field_position + 1                  as ordinal_position				-- 17 - INTEGER NOT NULL
      ,CASE (coalesce(rfr.rdb$null_flag,0,0)-1)*-1
            WHEN 0 THEN 'NO'
            WHEN 1 THEN 'YES'
                   ELSE 'UNKNOWN'
 ...
 WHERE rfr.rdb$field_source  = fld.rdb$field_name
   ...
   AND tbl.rdb$relation_name = 'DETAIL'
 ORDER BY rfr.rdb$relation_name
