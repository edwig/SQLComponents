...
DELIMITER $$
CREATE FUNCTION `MostExpensiveProduct`(p_year INTEGER)
...
DELIMITER $$
CREATE FUNCTION `CheapestProduct`(p_year INTEGER)
returns DECIMAL(10,2)
READS SQL data
BEGIN
...
DELIMITER $$
CREATE FUNCTION `ItemsOrderedBetween`(p_start DATE,p_end DATE)
RETURNS DECIMAL(10,2)
READS SQL data
BEGIN
...
DELIMITER $$
CREATE PROCEDURE `CorrectEmployees`()
BEGIN
   UPDATE `employees`
